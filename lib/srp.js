...
function getx(params, salt, I, P) {
  ...
  var hashIP = crypt.createHash(params.hash)
    .update(Buffer.concat([I, new Buffer(':'), P]))
    .digest();
  var hashX = crypt.createHash(params.hash)
    .update(salt)
    .update(hashIP)
    .digest();
...
function genKey(bytes, callback) {
  ...
  crypt.randomBytes(bytes, function(err, buf) {
...
function getu(params, A, B) {
  ...
  var u_buf = crypt.createHash(params.hash)
    .update(A).update(B)
    .digest();
...
function getK(params, S_buf) {
  ...
  return crypt.createHash(params.hash)
      .update(S_buf)
      .digest();
};
